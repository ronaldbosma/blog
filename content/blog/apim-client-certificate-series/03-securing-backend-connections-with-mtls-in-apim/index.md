...
summary: "In this third post on working with client certificates in Azure API Management, we'll focus on securing backend connections with mTLS. Using Bicep, we'll reference a client certificate stored in Key Vault, make it available in API Management and configure a backend resource that usess the certificate during the mTLS handshake."
---
