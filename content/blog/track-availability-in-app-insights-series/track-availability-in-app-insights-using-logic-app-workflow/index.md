...
Another approach to reduce costs is to run the availability tests less frequently in development and test environments. This means configuring the Recurrence trigger with interval and frequency values loaded from app settings (environment variables). At the moment, you can't configure this directly in the Logic App designer. You can configure it in the code view instead. Luis Rigueira has a great post about this: [Dynamic Logic App Schedules: Using Environment Variables](https://blog.sandro-pereira.com/2026/02/20/dynamic-logic-app-recurrence-environment-variables/).
